---
title: "Main publication figures for Nawaz et al"
author: "urwahnawaz"
date: "2024-10-08"
output:
...
This notebook contains code used to generate main figures in [Nawaz et al (2025)](https://www.biorxiv.org/content/10.1101/2025.06.12.659426v1). The code used to generate the mRNA-protein correlation data used for these figures can be found in  `analysis/calculate_correlations.Rmd`, or in the Analysis option on the option bar in this notebook. Figures which utilise the raw protein or RNA expression matrices can be found in the relevant publicaitons. 
...
# Figure 1 
mRNA-protein correlations across tissues and cancer related pathways. 
...
```{r fig.height=10, fig.width=20, fig.cap="Distribution of mRNA-protein correlations across tissues in the ProCan-DepMapSanger dataset. Only tissues with at least 10 cell lines are shown."}
...
```{r fig.height=4,fig.width=10, fig.cap="Histogram of mRNA-protein correlations across each cohort for ProCan-DepMapSanger, CPTAC and CCLE with medians indicated"}
...
```{r fig.height=11, fig.width=11, fig.cap="Boxplot showing median mRNA-protein correlations by tissue in the ProCan-DepMapSanger dataset for key biological and selected cancer-related pathways"}
...
```{r fig.height=8, fig.width=5, fig.cap="Correlation of median mRNA-protein correlations per tissue for CPTAC with ProCan-DepMapSanger (top), and with CCLE (bottom), for pathways in Figure 1d"}
...
pathways_cor_2 = median_pathway %>% 
 ...
 ggarrange(pathways_cor_1, pathways_cor_2, nrow=2)
...
mRNA-protein correlations of cancer driver genes from OncoKB.
...
```{r fig.height=6, fig.width=6, fig.cap=" Heatmap of mRNA-protein correlations across tissue types for cancer driver genes from OncoKB. Clusters identidied by hierachical clustering using Euclidean distance."}
...
```{r fig.height= 5, fig.width=5, fig.cap="Violin plot showing mRNA-protein correlations of cancer driver and non-driver genes with median and standard deviation indicated. **** indicates P < 0.0001 by unpaired Student’s t-test."}
...
```{r fig.height=10, fig.width=7, fig.cap="Distribution of mRNA-protein correlations for genes in each cluster identified in heatmap in Figure 2A"}
...
GO terms for each cluster were identified using the clusterProfiler and saved in a CSV. The top terms were combined with the distribution plot in Adobe Illustrator.
...
gene_ontology_top$cluster1 %>% 
  DT::datatable(caption = "Gene ontology for Cluster 1")
```

```{r}
gene_ontology_top$cluster2 %>% 
  DT::datatable(caption = "Gene ontology for Cluster 2")
```

```{r}
gene_ontology_top$cluster3 %>% 
  DT::datatable(caption = "Gene ontology for Cluster 3")
```

```{r}
gene_ontology_top$cluster4 %>% 
  DT::datatable(caption = "Gene ontology for Cluster 4")
```

```{r}
gene_ontology_top$cluster5 %>% 
  DT::datatable(caption = "Gene ontology for Cluster 5")
```
...
Integration of protein reproducibility ranks and gene-specific mRNA-protein correlations. Protein reproducibility ranks were retrieved from [Upadhya et al (2022)](https://www.cell.com/cell-reports-methods/fulltext/S2667-2375(22)00170-9?_returnURL=https%3A%2F%2Flinkinghub.elsevier.com%2Fretrieve%2Fpii%2FS2667237522001709%3Fshowall%3Dtrue). 
## Figure 3A
...
```{r fig.height=4, fig.width=4, fig.cap="Association between aggregated protein reproducibility ranks from Upadhya et al 22 and mRNA-protein correlations in the ProCan-DepMapSanger dataset."}
ggarrange(ranks_plot, correlation_ranks, nrow = 2)
```
...
```{r fig.height=6, fig.width=6, fig.cap=" Scatter plot of mRNA-protein correlation against aggregated protein reproducibility rank, with cancer drivers indicated by triangle data points."}
library(ggpubr)
...
```{r fig.height=10, fig.width=5, fig.cap="mRNA-protein correlation of CTNNB1 for each tissue type. "}
correlation_data_tissue = read.csv("output/correlations-datasets/correlation-tissue-nontransformed.csv", row.names = 1)
...
```{r fig.height=6, fig.width=6, fig.cap= "Distribution of CTNNB1 mRNA expression across samples from the ProCan-DepMapSanger dataset, with outliers (> 3 standard deviations above the mean) indicated"}
library(ggbeeswarm)
...
```{r fig.height=8, fig.width=6, fig.cap="Scatterplot showing mRNA expression and protein abundance for CTNNB1 (upper) and distribution of CTNNB1 protein abundance (lower) across samples in the ProCan-DepMapSanger dataset."}
ggarrange(ctnnb1, protein_swarm, nrow = 2)
```
